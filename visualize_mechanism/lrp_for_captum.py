...
from captum.attr._utils.lrp_rules import EpsilonRule, PropagationRule, GammaRule

## Add by myself
from models.tcn import Chomp1d, AdditionModule
from models.basic_blocks import Gmp_softmax_block, Conv1d_block, LastStepModule
from models.unet import Utime
from models.mlp import Linearblock, Unsqueeze
from models.unet import Encoder, Decoder, SegmentClassifier
...
class LRP(GradientAttribution):
    ...
    def attribute(
        ...
        Args:
            ...
            rule_dict (dict): A propagation rule which the layers follow
                                                Either Epsilon or Gamma rule
    ...
    def compute_convergence_delta(self, attributions, output):
        ...
        print("[LRP] delta: ", delta)
        return delta

    def _get_layers(self, model):
        model_has_layers_attr = False
        for key in model.__dict__.keys():
            if key == "layers":
                model_has_layers_attr = True
        if model_has_layers_attr:
            layers = model.layers
        else:
            #if model.model_name() in ["MLP"]:
                
            if type(model.model) == Utime:
                layers = model.model
            else:
                layers = model.model.layers
        if model.model_name() in ["TCN", "FCN"]:
            for layer in layers:
                if isinstance(layer, Gmp_softmax_block) or isinstance(layer, Conv1d_block):
                    last_l = None
                    for idx, l in enumerate(layer.modules()):
                        if isinstance(l, nn.Sequential):
                            for num, le in enumerate(l.modules()):
                                if type(le) in SUPPORTED_LAYERS_LIST:
                                    self.layers.append(le)
                        if isinstance(last_l, nn.Linear):
                            self.layers.append(last_l)
                        last_l = l
                else:
                    for idx, l in enumerate(layer.modules()):
                        if isinstance(l, nn.Sequential) and idx == 1:
                            for num, le in enumerate(l.modules()):
                                if type(le) in SUPPORTED_LAYERS_LIST:
                                    self.layers.append(le)
        elif model.model_name() in ["MLP"]:
            for layer in layers:
                if isinstance(layer, Linearblock):
                    ...
                    for num, le in enumerate(layer.modules()):
                        if type(le) in SUPPORTED_LAYERS_LIST:
                            self.layers.append(le)
                    ...
                    #     if type(le) in SUPPORTED_LAYERS_LIST:
                    self.layers.append(le)
                elif isinstance(layer, Unsqueeze):
                    # for num, le in enumerate(layer.modules()):
                    #     if type(le) in SUPPORTED_LAYERS_LIST:
                    self.layers.append(le)
        elif type(model.model) != Utime:
            for layer in layers:
                for idx, l in enumerate(layer.modules()):
                    if isinstance(l, nn.Sequential) and idx == 1:
                        for num, le in enumerate(l.modules()):
                            if type(le) in SUPPORTED_LAYERS_LIST:
                                self.layers.append(le)
                    elif layer == layers[-1] and isinstance(l, nn.Sequential):
                        for num, le in enumerate(l.modules()):
                            if type(le) in SUPPORTED_LAYERS_LIST:
                                self.layers.append(le)
        elif type(model.model) == Utime:
            for layer in layers.modules():
                if isinstance(layer, Encoder):
                    for block, maxpool in zip(layer.blocks, layer.maxpools):
                        for idx, l in enumerate(block.modules()):
                            if isinstance(l, nn.Sequential) and idx != 0:
                                for num, le in enumerate(l.modules()):
                                    if type(le) in SUPPORTED_LAYERS_LIST:
                                        self.layers.append(le)

                        if type(maxpool) in SUPPORTED_LAYERS_LIST:
                            self.layers.append(maxpool)
                    for botto in layer.bottom:
                        for idx, l in enumerate(botto.modules()):
                            if isinstance(l, nn.Sequential) and idx != 0:
                                for num, le in enumerate(l.modules()):
                                    if type(le) in SUPPORTED_LAYERS_LIST:
                                        self.layers.append(le)
                if isinstance(layer, Decoder):
                    for upsample, block in zip(layer.upsamples, layer.blocks):
                        for idx, l in enumerate(upsample.modules()):
                            if isinstance(l, nn.Sequential) and idx != 0:
                                for num, le in enumerate(l.modules()):
                                    if type(le) in SUPPORTED_LAYERS_LIST:
                                        self.layers.append(le)
                        for idx, l in enumerate(block.modules()):
                            if isinstance(l, nn.Sequential) and idx != 0:
                                for num, le in enumerate(l.modules()):
                                    if type(le) in SUPPORTED_LAYERS_LIST:
                                        self.layers.append(le)
                if isinstance(layer, SegmentClassifier):
            ...
            for layer in layers.children():
                if len(list(layer.children())) == 0:
                    ...
                    self._get_layers_children(layer)
    ...
    def _get_layers_children(self, layers):
        for layer in layers.children():
            if len(list(layer.children())) == 0:
                self.layers.append(layer)
            else:
    ...
    def _check_and_attach_rules(self):
        if self.rule_dict is None:
            self.rule_dict = SUPPORTED_LAYERS_WITH_RULES
        for layer in self.layers:
            if hasattr(layer, "rule"):
                pass
            elif type(layer) in self.rule_dict.keys():
                layer.rule = self.rule_dict[type(layer)]()
...
SUPPORTED_LAYERS_WITH_RULES = {
    ...
    nn.AdaptiveMaxPool1d: EpsilonRule,
    nn.ZeroPad2d: EpsilonRule,
    Chomp1d: EpsilonRule,
    AdditionModule: EpsilonRule
}

SUPPORTED_NON_LINEAR_LAYERS = [nn.ReLU,
                               nn.Dropout,
                               nn.Tanh,
                               # models.tcn.Chomp1d
                               LastStepModule,
...
SUPPORTED_LAYERS_LIST = [nn.Conv1d,
                         nn.BatchNorm1d,
