...
def aggregate_metric(dataset, model, perspective, mean_across_dataset=True):
    ...
    for experiment_index in range(12):
        ...
        if perspective == "faithfulness_ti":
            ...
            # Temporal Importance
            filename = f"temporal_importance/{model}_{dataset}_faithfulness_ti_eval_mean_0.2_predscores.h5"
            data = read_h5(directory + filename)
            if data is None:
                continue
            # print(data)
            ...
            for visualization in data.index:
                ...
                gap_scores = inner_data["importance"]  # - inner_data["unimportance"]
            ...
            filename = (
                f"class_sensitivity/{model}_{dataset}_sensitivity_cs_correlation.npy"
            )
        ...
        # TODO: This is now robustness.
            ...
            filename = (
                f"intraclass_stability/{model}_{dataset}_stability_intraclass_dtw.h5"
            )
            data = read_h5(directory + filename)
            if data is None:
                continue
            ...
            filename = f"sanity_check/{model}_{dataset}_sanity_Cascade_ssim_final.npy"
            # filename = "sanity_checkFCN_withoutFC_FordA_sanity_Cascade_random_saliencymaps_fcn.0.conv1_abs.npy"
            data = read_results(directory + filename)
            if data is None:
                continue
...
def read_data():
    ...
    for dataset, model, perspective in itertools.product(
        ...
        results = aggregate_metric(dataset, model, perspective)
        if results is not None:
            ...
            print(f"[Normal Data] Missing Data for {dataset}/{model}/{perspective}.")

    ## random baseline
    for dataset, model, perspective in itertools.product(
        DATASETSRANDOM, MODELSRANDOM, PERSPECTIVES
    ):
