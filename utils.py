...
def throw_out_wrong_classified(model: nn.Module,
    ...
    print(f"[INFO] The count of classes is {np.unique(predicted, return_counts=True)}")
    if len(np.unique(predicted)) <= 1:
        print(f"[ERROR] there has to me more than one class left")
        exit(1)
...
def load_saliencies(path: str, experiments: List[str], randombaseline=True, filenames_appendix: List=None):
    ## Load the Saliency Maps
    dataset_name, model_name, norm_regularize, norm_parameter = filenames_appendix
    
    ## Abs Norm and No Abs Norm
    saliencymaps_abs_norm_list = []
    saliencymaps_no_abs_norm_list = []
    if randombaseline:
        random_saliencymaps_list = []
    for experiment in experiments:
        ...
        if not randombaseline:
            abs_norm_name = path + "saliencymaps_abs_norm__{}_{}_{}_{}_".format(dataset_name, model_name, norm_regularize, norm_parameter) + experiment + ".npy"
            no_abs_norm_name = path + "saliencymaps_no_abs_norm__{}_{}_{}_{}_".format(dataset_name, model_name, norm_regularize, norm_parameter) + experiment + ".npy"
            ...
            randombaseline_name = path + "randombaseline_map" + experiment + ".npy"
            random_maps = np.load(randombaseline_name, allow_pickle=True)
            random_saliencymaps_list.append(random_maps.item())
    return (saliencymaps_abs_norm_list, saliencymaps_no_abs_norm_list, random_saliencymaps_list) if randombaseline else (saliencymaps_abs_norm_list,                  saliencymaps_no_abs_norm_list)
